...
<body>
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr class="<?= $rowClass ?>">
                                ...
                                <td>
                                    <span class="material-icons-sharp warning edit-task" data-task-id="<?php echo $row['id']; ?>" data-nama-divisi="<?php echo $row['nama_divisi']; ?>">
                                        edit
                                    </span>
                                </td>
                                <td>
    ...
    <div class="modal">
        <div class="modal-content">
            <h2 class="modal-title">Tambah Divisi Baru</h2>
    ...
    <div class="edit-modal">
        <div class="modal-content">
            <h2 class="modal-title">Edit Nama Divisi</h2>
            <span class="close-button">&times;</span>
            <form method="get" action="../edit/edit_divisi.php">
                <label for="nama_divisi">Nama Divisi:</label>
                <input type="text" id="edit_nama_divisi" name="nama_divisi" autocomplete="off" required>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const editNamaDivisiInput = editModal.querySelector('#edit_nama_divisi');
            const editIcons = document.querySelectorAll('.edit-task');

            editIcons.forEach(icon => {
                icon.addEventListener('click', function() {
                    const id = this.getAttribute('data-task-id');
                    const namaDivisi = this.getAttribute('data-nama-divisi');

                    editNamaDivisiInput.value = namaDivisi;
        ...
        function confirmDelete(taskId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const deleteUrl = `../delete/hapus_data.php?method=divisi_delete&id=${taskId}`;
                    fetch(deleteUrl)
                        .then(response => {
                            if (response.ok) {
                                location.reload();
